...
public class Program
{
    public static int Main(string[] args)
    {
        ...
        app.Configure(config =>
        {
            ...
            config.AddExample(["generate", "--wsdl", "path/to/service.wsdl", "--output", "output/directory"]);
            config.AddExample(["generate", "--wsdl", "path/to/service.wsdl", "--output", "output/directory", "--namespace", "MyCompany.SoapClient"]);
...
public class GenerateCommand : AsyncCommand<GenerateCommandSettings>
{
    public override async Task<int> ExecuteAsync(CommandContext context, GenerateCommandSettings settings)
    {
        ...
        var logger = loggerFactory.CreateLogger<DotnetClientGenerator.SoapClientGenerator>();
        ...
        return await AnsiConsole.Status()
            .StartAsync("Generating SOAP client...", async ctx =>
            {
                try
                {
                    ...
                    var generator = new DotnetClientGenerator.SoapClientGenerator(options, logger);
